...
export function LocalChatMessageList({ listRef, loadingHistory, messages, sending }: Props) {
  return (
    <div ref={listRef} className="flex-1 overflow-y-auto bg-neutral-50/60 px-5 py-4">
      {loadingHistory ? (
        <div className="rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-500">正在加载历史对话...</div>
      ) : messages.length === 0 ? (
            ...
            <div key={`${m.role}-${m.ts}-${idx}`} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[82%] rounded-2xl border px-4 py-3 text-sm leading-7 shadow-sm ${
                  m.role === "user" ? "border-blue-200 bg-blue-50 text-blue-900" : "border-neutral-200 bg-white text-neutral-800"
                }`}
              >
                {m.role !== "assistant" ? (
