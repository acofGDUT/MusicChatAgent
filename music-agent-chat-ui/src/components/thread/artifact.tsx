...
const ArtifactSlotContext = createContext<ArtifactContextValue | null>(null);

function useArtifactSlotContext(): ArtifactContextValue {
  const context = useContext(ArtifactSlotContext);
  if (!context) {
    throw new Error("Artifact hooks/components must be used within an ArtifactProvider.");
  }
  return context;
}
...
const ArtifactSlot = (props: {
  ...
  const context = useArtifactSlotContext();
