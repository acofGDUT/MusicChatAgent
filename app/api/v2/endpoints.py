...
async def add_demo_song():
    return await songlist.add_songs(201, [97773])
...
async def get_playlist_browser_page(dirid: int, page: int = Query(1, ge=1), page_size: int = Query(10, ge=1, le=50), playlist_name: str = Query("", description="可选：前端已知歌单名")):
    res = await playlist_service.get_playlist_detail(dirid=dirid, page=page, num=page_size, onlysong=False)
    ...
    for idx, song in enumerate(playlist_songlist):
        ...
        if not song_mid:
            continue
        tracks.append({"index": (page - 1) * page_size + idx + 1, "song_mid": song_mid, "title": str(song.get("title", "") or "未知歌曲"), "artist": str(song.get("singer", "") or ""), "cover": ""})

    has_more = page * page_size < total_song_num
    return {"type": "playlist_browser", "playlist_name": resolved_name, "dirid": dirid, "page": page, "page_size": page_size, "total_song_num": total_song_num, "has_more": has_more, "tracks": tracks, "description": f"已加载第 {page} 页，可点击歌曲直接播放"}


@router.get("/song/play-url")
async def get_play_url_by_song_mid(song_mid: str = Query(..., description="歌曲 mid"), title: str = Query("", description="可选：歌曲标题"), artist: str = Query("", description="可选：歌手名")):
    ...
    return {"type": "play_music", "song_mid": clean_mid, "title": title, "artist": artist, "url": playable_url, "description": "已获取播放链接"}
