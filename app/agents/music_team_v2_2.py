...
def _supervisor_pre_model_hook(state: Any) -> dict[str, Any]:
    ...
    looks_done = any(k in content for k in ["已完成", "操作成功", "成功", "已添加", "已创建", "查询结果", "\"type\": \"play_music\"", "```json"])
...
# --- B. 音乐播放专员 (PlayAgent) ---
playagent_tools = [search_music_tool, get_created_songlist_tool, get_playlist_detail_tool, play_music_tool]
playagent_prompt = """
...
replier_prompt = (
    ...
    "2) 若上游 PlayAgent 返回的是 `type=play_music` 的 JSON：优先原样输出该 JSON（可使用 ```json 代码块），不要转成 HTML。\n"
