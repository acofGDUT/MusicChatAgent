...
def _same_message(left: Any, right: Any) -> bool:
    if left is right:
        return True
    left_type = left.get("type", "") if isinstance(left, dict) else getattr(left, "type", "")
    right_type = right.get("type", "") if isinstance(right, dict) else getattr(right, "type", "")
    return left_type == right_type and msg_content(left) == msg_content(right)


def _matching_input_prefix_len(result_messages: list[Any], input_messages: list[BaseMessage]) -> int:
    max_len = min(len(result_messages), len(input_messages))
    for length in range(max_len, 0, -1):
        input_tail = input_messages[-length:]
        if all(_same_message(result_messages[index], input_tail[index]) for index in range(length)):
            return length
    return 0


def extract_agent_delta_messages(
    result: Any,
    input_messages: list[BaseMessage],
    fallback_name: str,
) -> list[Any]:
    """Return only messages generated by an agent invocation.

    LangChain agents commonly return the input messages plus tool/AI outputs.
    Graph nodes should only send the new delta to LangGraph's add_messages
    reducer, while preserving ToolMessage payloads for artifact collection.
    """
    if not isinstance(result, dict):
        return [extract_last_ai_message(result, fallback_name)]

    result_messages = result.get("messages", [])
    if not isinstance(result_messages, list) or not result_messages:
        return [extract_last_ai_message(result, fallback_name)]

    start_index = _matching_input_prefix_len(result_messages, input_messages)
    delta_messages = result_messages[start_index:]
    if not delta_messages:
        return [extract_last_ai_message(result, fallback_name)]

    for message in delta_messages:
        if isinstance(message, AIMessage) and not getattr(message, "name", None):
            message.name = fallback_name
