...
- 状态：Done（2026-07-16；已集成 architecture 本地聊天主线并完成全量离线验收）
- 优先级：P0
...
- 最终后端测试数量：237 passed；完整 `.venv/Scripts/python.exe -m pytest -q` 无排除项通过
- 原 architecture 分支的 Artifact、message delta、API error contract 和 search service 测试已迁移到新 lifespan/身份合同并纳入全量测试，不再存在旧模块收集错误
- 前端检查结果：3 service tests passed；`tsc --noEmit` 通过；chat-local Prettier check 通过；Next lint 0 error（有既有 warnings）；production build 通过
...
- 遗留问题：当前 SQLite 未加密且仅支持本地单账号逻辑隔离；真实 QQ/LLM 浏览器 E2E 尚未执行
- Git commit：Phase 3 已保存为 `00fc9ca`，随后合入 architecture 集成分支
